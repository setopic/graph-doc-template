...
- **A007 は数えてから指摘する。** 「理由は 3 つある」のように**個数を宣言し、
  ...
  （機械的に当てると、この取り違えで誤検出が本物を上回った）
- **A008・A009 は、文書が「こう作る」と規定している箇所だけが対象。**
  「将来に備えて」「汎用的に」のような語が出てくるだけでは指摘しない。
  次の 3 つは対象外:
  - ADR の「検討した案」「却下した案」。案を並べるのは ADR の仕事である
  - 「やらない」「必要になったら足す」と**先送りを明記した**箇所。
    過剰な設計を避けた記録であって、兆候ではない
  - 一般化や抽象が要る理由（2 つ目の実体がある、受け入れ条件が求めている）が、
    本文か、本文のリンクの題名から読み取れるもの
- **A009 は具体を数えてから指摘する。** 差し替えられる仕組み（インターフェース、
  基底、戦略、プラグイン）に対して、本文が挙げている具体が 1 つだけのときに限る。
  契約（type: contract）が境界の約束を定めることは、使う側が 1 つでも対象外
- **A008・A009 は断定しない。** 依存先の本文は渡していないので、根拠が依存先にだけ
