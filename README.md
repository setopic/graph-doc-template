...
循環依存・**層の逆流**を機械検証する。外部依存なし（Python 3.12+ の標準ライブラリのみ）。
...
`G009`〜`G015` と `G017`・`G020`・`G022`・`G023` は**健全性の警告**で、承知のうえで放置してもよい
（`--strict` で失敗扱いにできる）。
`G018` は**近づいていれば警告、達していればエラー**（その時点で図は描画されていない）。
**`G019` と `G021` はエラー**（切れた行は既にただの文字列として出ており、
自動ブロックの下に回った本文は読む人に届いていない）。
しきい値と必須の節は `tools/graph/schema.py` にある。

**`G018` / `G019` / `G021` は「見た目」と「届くか」を見ている。**
グラフの整合性は全部通るのに、読む人のところで壊れている、という種類の問題が
実際に起きたため。**`G021` は `sync` が直せない**（ブロックはその場で
入れ替えるだけで、後ろに回った本文は動かさない）。

**`G015` と `G016` / `G017` は使う場面が違う。** `G015` は文書どうしの追従漏れ、
`G016` / `G017` は文書と実装の対応（`implemented_by` を書いたときだけ効く）。
**どちらも「見たか」を確かめるもので、「直せ」ではない。**

**`G020` は `G009` と重ならない。** `G009` は `stable` なノードの `depends_on` だけを
見る。`G020` は**本文のリンク**を見て、取り下げた決定を断りなく引いていないかを
確かめる。**同じ段落で置き換え先も指していれば黙る。**

**`G020` は確定した記録を見ない**（`schema.py` の `IMMUTABLE_RECORD_TYPES`、既定は
`stable` な ADR）。**確定した ADR は書き換えない**もので、決定を変えるときは
新しい ADR を起こす。**直せないものを鳴らしても `--strict` を塞ぐだけである。**
確定前（`draft` / `review`）は直せるので対象に残る。
見るのは実質、現在の設計を述べる層だけになる。

**CI では main への push と PR で `--strict` を使う**（PR で落ちなければ、マージした後の main も落ちない）。
PR の `--strict` は `--since` を付けずに走らせるので、`G015`・`G017` は失敗にならない（一覧だけ出す）。
それ以外のブランチへの push では警告を出すだけにして、書いている途中で止めない。
