...
切り戻し / 未確認」を持つ。**手順の中の順序はグラフに乗らないので、本文の
「順序の制約」に書く**（[META-01](./graph-rules.md) の「順序はグラフに乗らない」）。

### 「切り戻し」を空欄にしない

**「戻せない」は最も重要な答え**で、着手する前に知っておく必要がある。
実際に書いてみると、戻せない手順は珍しくない（発行し直したトークンは元に戻らない、
削除したインスタンスは復元できない）。どこを過ぎたら戻せなくなるかまで書く。
