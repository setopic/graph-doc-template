...
| 時間帯 | 開始 / 終了 | ○ | 開始 < 終了。分単位 |
| 状態 | 列挙 | ○ | 仮予約 / 確定 / 取消 |
| 保持期限 | 時刻 | — | 「仮予約」のときだけ持つ。これを過ぎると確定できない |
...
**「仮予約」だけが保持期限を持つ。** 期限を過ぎた仮予約は確定できない。

「取消」への遷移が起きるのは、**期限切れを検出した時点**である（確定を要求されて
期限を過ぎていると分かったとき、など）。**期限を過ぎた瞬間に状態が変わるのではない。**
状態が「仮予約」のまま期限だけ過ぎている予約は存在しうる。
